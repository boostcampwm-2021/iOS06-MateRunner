...
final class RunningUseCaseTests: XCTestCase {
    ...
    func test_execute_cancel_timer_decrease_five() {
        self.scheduler.createHotObservable([
            ...
            self?.useCase.executeCancelTimer()
        }).disposed(by: self.disposeBag)
    ...
    func test_execute_pop_up_timer_should_show_pop_up() {
        self.scheduler.createHotObservable([
            ...
            self?.useCase.executePopUpTimer()
        }).disposed(by: self.disposeBag)
    ...
    func test_invalid_cancel_timer() {
        self.scheduler.createHotObservable([
            ...
            self?.useCase.invalidateCancelTimer()
        }).disposed(by: self.disposeBag)
        ...
        XCTAssertEqual(numberTestableObserver.events, [
            .next(10, 3)
        ])
    }
